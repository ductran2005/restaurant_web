...
public class PreOrderPageController extends HttpServlet {
        ...
        try {
            switch (action) {
                ...
                case "remove":
                    ...
                    break;
                case "confirm":
                    // Save items then redirect to checkout
                    handleConfirm(req);
    ...
    private void handleConfirm(HttpServletRequest req) {
        ...
        try {
            ...
            // Delete existing items to replace with new cart
            s.createMutationQuery("DELETE FROM PreOrderItem WHERE booking.id = :bookingId")
             .setParameter("bookingId", booking.getId())
             .executeUpdate();

            String itemCountStr = req.getParameter("itemCount");
            int count = (itemCountStr != null && !itemCountStr.isBlank()) ? Integer.parseInt(itemCountStr) : 0;
            
            for (int i = 0; i < count; i++) {
                String productIdStr = req.getParameter("productId_" + i);
                String qtyStr = req.getParameter("quantity_" + i);
                if (productIdStr != null && qtyStr != null) {
                    Product product = productDao.findById(s, Integer.parseInt(productIdStr));
                    if (product != null) {
                        PreOrderItem poi = new PreOrderItem();
                        poi.setBooking(booking);
                        poi.setProduct(product);
                        poi.setQuantity(Integer.parseInt(qtyStr));
                        s.persist(poi);
                    }
                }
            }

...
public class UserPreOrderCheckoutController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            // Debug logging
            System.out.println("=== User Pre-order Checkout Debug ===");
            System.out.println("Booking Code: " + booking.getBookingCode());
            System.out.println("Pre-order items count: " + items.size());
            for (PreOrderItem poi : items) {
                if (poi != null && poi.getProduct() != null) {
                    System.out.println("  - " + poi.getProduct().getProductName() + 
                        " x" + poi.getQuantity() + 
                        " = " + poi.getProduct().getPrice().multiply(new BigDecimal(poi.getQuantity())));
                }
            }
